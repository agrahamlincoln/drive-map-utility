...
namespace drive_map_utility
{
	...
	public class NetworkDrive
    {
        ...
        public NetworkDrive(string letter, string path)
        {
            this.ShareName = path;
            this.LocalDrive = letter;
        }

        public NetworkDrive(string path)
        {
            this.ShareName = path;
        }
        #endregion
		...
		private void zMapDrive(string psUsername, string psPassword){
			...
			if(i>0){
                Utilities.writeLog("Attempt to map: " + ls_ShareName + " on drive " + ls_Drive + " has failed.");
